<div class="flex gap-1 justify-center">
    <a href="{{ url('gestion-reservas') }}"
        class="relative px-4 py-1.5 text-sm text-[--color-secundario] transition hover:bg-[--color-eventos] rounded-md duration-300 ease-in-out group {{ Request::is('gestion-reservas') ? 'bg-[--color-eventos]' : '' }}">
        Reservas
    ...
    <a href="{{ url('gestion-salas') }}"
        class="relative px-4 py-1.5 text-sm text-[--color-secundario] transition hover:bg-[--color-eventos] rounded-md duration-300 ease-in-out group {{ Request::is('gestion-salas') ? 'bg-[--color-eventos]' : '' }}">
        Gestión Salas
        <span class="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-[3px] bg-[--color-primario] rounded-full transition-all duration-300 ease-in-out group-hover:w-3/4"></span>
    </a>
    <span class="text-[--color-primario] font-semibold">|</span>
